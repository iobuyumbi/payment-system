...
const LoanPortfolioReport: React.FC<ReportProps> = ({ title, reportData }) => {
 ...
 const [actions, setActions] = useState<any>();
  ...
  const bindApplications = async (props: any) => {
    try {
      const response = await reportsService.getApplicationReports(props);
  ...
  const handleExcelExport = async () => {
    const exportExcel = async () => {
      ...
      for (const batchId of loanProductIds) {
        const selectedBatch = loanBatches.find((b: any) => b.value === batchId);
        ...
        const mappedData = filteredData.map((row: any) => {
          ...
          const totalArrearsVal = row.totalArrears ?? 0;
          const totalExpectedVal = row.totalExpected ?? 0;

          totalDisbursed += disbursed;
          totalPrincipalDue += principalDue;
          totalPrincipalReceived += principalReceived;
          totalPrincipalBalance += principalBalance;
          totalPrincipalArrears += principalArrears;
          totalInterest += interest;
          totalInterestDue += interestDue;
          totalInterestReceived += interestReceived;
          totalInterestArrears += interestArrears;
          totalArrears += totalArrearsVal;
          totalExpected += totalExpectedVal;

          return {
            "Loan number": row.loanNumber,
            "Disbursed date": moment(row.disbursedOn).format(
              "YYYY-MM-DD HH:mm"
            ),
            "Date of Witness": row.dateOfWitness
              ? moment(row.dateOfWitness).format("YYYY-MM-DD HH:mm")
              : "N/A",
            ...
            "Farmer System Id": row.farmer.systemId,
            "Farmer name": row.farmer.firstName + " " + row.farmer.lastName,
            ...
            "Total Arrears": totalArrearsVal,
            "Total Expected": totalExpectedVal,
            "Arrears Rate [%]": row.arrearsRate,
          };
        });
        ...
        mappedData.push({
          ...
          "Disbursed date": filteredData[0].currentUserName,
        });
      ...
      const filename = `Loan_portfolio_report_${timestamp}.xlsx`;
      ...
      await toast.promise(
        (async () => {
          ...
          const response = await axios.post(
            `${getAPIBaseUrl()}api/Reports/GetLoanApplicationReportsPdf`,
            data,
            {
  ...
  const downloadPdf = async () => {
    ...
    await toast.promise(
      (async () => {
        ...
        a.href = fileUrl;
        a.download = `LoanPortfolioReport_${timestamp}.pdf`;
