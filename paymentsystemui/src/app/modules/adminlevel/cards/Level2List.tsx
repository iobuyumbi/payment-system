...
const AdminLevel2List: FC<any> = (props: any) => {
  const navigate = useNavigate();
  const { setSubCountyData, countyId } = props
  ...
  const editButtonHandler = (value: any) => {
    setSubCountyData(value)
  };

  const afterConfirm = (res: any) => {
    if (res) bindAdminLevelData(countyId);
  ...
  const openDeleteModal = (id: any) => {
    if (id) {
      const confirmModel: IConfirmModel = {
        ...
        deleteUrl: `api/adminlevel2/${id}`,
        message: "delete-subcounty",
      };
  ...
  const [colDefs, setColDefs] = useState<any>([
    { field: "subCountyName", headerName: "Admin level name", flex: 1 },
    { field: "subCountyCode", headerName: "Admin level code", flex: 1 },
 { field: "countyName", flex: 1 },
  ...
  const bindAdminLevelData = async (props: any) => {
    if (props !== undefined) {
      ...
      const response = await adminLevelService.getAdminLevel2Data(
        pageData
      );
      setRowData(response);
    }
...
export default AdminLevel2List;
