...
import PaymentDeductibleForm from "./PaymentDeductibleForm";
...
export function AddDeductiblePayments() {
 const {id}= useParams()
    ...
    <Content>
      {isAllowed("payments.batch.add") ? (
        <>
          {" "}
          <PageTitleWrapper />
          <PageTitle breadcrumbs={breadCrumbs}>{title} Deductible Payment</PageTitle>
          <KTCard className="my-3">
            <KTCardBody>
              <PaymentDeductibleForm  />
            </KTCardBody>
          </KTCard>
